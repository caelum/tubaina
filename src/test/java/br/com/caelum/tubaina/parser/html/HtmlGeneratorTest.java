...
public class HtmlGeneratorTest {
	...
	@Test
	public void testGeneratorWithDuppedSectionName() throws TubainaException, IOException {
		BookBuilder builder = new BookBuilder("teste");
		builder.add(new StringReader("[chapter qualquer um]\n"
				+ "alguma coisa\n[section duplicada]outra coisa\n[section duplicada]mais uma coisa\n"
				+ "[section duplicada]blah blah blah"));
		Book b = builder.build();
		...
		File chapter = new File(temp, "teste/qualquer-um");
		Assert.assertTrue(chapter.exists());

		File duplicada = new File(chapter, "duplicada");
		Assert.assertTrue(duplicada.exists());

		File duplicada2 = new File(chapter, "duplicada-2");
		Assert.assertTrue(duplicada2.exists());

		File duplicada3 = new File(chapter, "duplicada-3");
		Assert.assertTrue(duplicada3.exists());
	}
