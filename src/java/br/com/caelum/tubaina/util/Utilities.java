...
public class Utilities {
	...
	public static void getImageWithLogo (File srcImage, ImageOutputStream destinationImage, String format, File logo) {
		try {
		    ...
		    graphics.drawImage(image2, image.getWidth() - w, image.getHeight() - h, rectangleWidth, rectangleHeigth, null);
	...
	public static String getFormatName(File srcImage) {
		return FilenameUtils.getExtension(srcImage.getPath()).toUpperCase();
	}

	public static int getImageWidth(File srcImage) {
		try {
	...
	public static void resizeImage (File srcImg, ImageOutputStream destImg, String format, int pageWidth, double ratio){
		if (ratio <= 0 || ratio > 1) {
	...
	public static int getMinIndent(String string) {
	...
	public static int maxLineLength(String string) {
		int max = 0;
		for (String s : string.split("\n")) {
			if (s.length() > max) {
				max = s.length();
	...
	public static String toDirectoryName(Integer num, String title) {
